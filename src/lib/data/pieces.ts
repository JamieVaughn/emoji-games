...
const mapBuilder = () => (size: number) => {
  ...
  let indices = [
    ...
    (d: number) => d * 2,
    (d: number) => d ** 2 - d * 2 - 1,
    (d: number) => Math.floor(d / 2 - 1),
    (d: number) => d ** 2 - Math.floor(d / 2 + 1),
    (d: number) => Math.floor(d * 2.5),
    (d: number) => d ** 2 - Math.floor(d * 2.5),
    (d: number) => Math.floor(d * 4 - 1),
    (d: number) => d ** 2 - Math.floor(d * 4),
  ];
  return indices
    .map((f) => f(d))
