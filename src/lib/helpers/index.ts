...
export function draw(now, {then = now, delta = 0, drawCallback, fps = 30}) {
    ...
    const rAF = requestAnimationFrame(timestamp => draw(timestamp, {then, delta: now - then, drawCallback, fps}));
...
export function handleFallback<T>(item: T, index:number): Element {
